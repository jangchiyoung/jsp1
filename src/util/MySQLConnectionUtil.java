...
public class MySQLConnectionUtil {
	//MySql 연결하는 메소드를 작성. - 리턴 타입 Connection
	//웹애플리케이션 전체에서 사용할 라이브러리 : WEB-INF 폴더 -lib 폴더로 드래그 하여 복사하세요
	public static Connection connect() {
		...
		try {
			...
			//DriverManager 클래스의 메소드 getConnection 으로 데이터 베이스 드라이버에 대한 Connection 구현객체를 생성
	...
	public static void close(Connection conn) {
		if(conn != null)	//conn이 널이 아닐때만 close 실행하도록합니다.
			try {
				conn.close();
			} catch (SQLException e) {
